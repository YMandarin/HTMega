...
constant button_hold_time_12Mhz : integer := 24000000;
signal button_hold_counter : integer range 0 to button_hold_time_12Mhz := 0;
...
if falling_edge(clk_12) then
	if button_hold = '1'then
		if button = '0' then
			...
			if button_hold_counter = (button_hold_time_12Mhz - 1) then
				button_hold_counter <= 0;
				button_hold <= '0';
				boot_mode_buffer <= '1';
			else
				button_hold_counter <= button_hold_counter + 1;
			end if;
		end if;
	end if;
