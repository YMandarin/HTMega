...
byte[] end_sequence = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
...
{
    if (StrParser.ToInt16(line, out short addr))
        return BitConverter.GetBytes(addr);
    else
        throw new ValueError($"expected program memory address of type int16, " +
            $"got {line}", i);
}
