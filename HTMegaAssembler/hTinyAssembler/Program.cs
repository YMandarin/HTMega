...
Dictionary<string, OpCode> OPCodes = new Dictionary<string, OpCode>()
{
    ...
    { "END",  new OpCode{ byteCode = 0b00000100, param = Param.none } },
...
void main()
{
            ...
            switch (op.param)
            {
                case Param.none:
                    if (parameters.Length > 0)
                        throw new SyntaxError($"no parameters allowed for: {mnemonic}", i);
    ...
    stopwatch.Stop();
    Console.WriteLine($"-------- finished, elapsed time: {stopwatch.ElapsedMilliseconds}ms ---------");
}
...
string parsePath(string path, string[] allowedExtensions, bool isInput)
{
    ...
    if (!isAllowed)
        throw new ArgumentError($"wrong file type of {path}, expected: {formattedExtensions}");
...
byte[] parsePMAddr(string line, int i)
{
    if (StrParser.ToUInt16(line, out ushort addr))
    {
        if (addr < PM_SIZE)
            ...
            throw new ValueError($"program memory address {line} exceeds program memory size of {PM_SIZE}", i);
    }
    else
        throw new ValueError($"expected program memory address of type uint16, got {line}", i);
}
...
byte[] parseRAMAddr(string line, int i)
{
    if (StrParser.ToUInt16(line, out ushort addr))
        if (addr < PM_SIZE)
            ...
            throw new ValueError($"RAM address {line} exceeds RAM size of {RAM_SIZE}", i);
    else
        throw new ValueError($"expected RAM address of type uint16, got {line}", i);
}
...
void testParameters(string[] parameters, int expectedParams, int i)
{
    if (parameters.Length != 2)
        throw new SyntaxError($"wrong amount of parameters: {parameters.Length}; expected amount of parameters: {expectedParams}", i);
...
public class ValueError : Exception
{
    ...
    public ValueError(string message, int line) : base($"at line {line}: {message}") { }
...
public class SyntaxError : Exception
{
    ...
    public SyntaxError(string message, int line) : base($"at line {line}: {message}") { }
...
public class ArgumentError : Exception
{
    ...
    public ArgumentError(string message, Exception inner) : base(message, inner) { }
    public ArgumentError(string message, int line) : base($"at line {line}: {message}") { }
